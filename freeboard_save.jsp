...
<!-- form에서 넘어오는 데이터는 모두 String으로 넘어옴
	Integer.perseInt()
	Double.perseDouble()
-->
...
<%
	...
	//out.println(yymmdd);	//Thu Jan 12 11:16:29 KST
	...
	if(!(rs.next())) {	 //테이블의 값이 존재하지 않는 경우
		...
		id = rs.getInt(1) + 1;		//getString으로 가져오면 함수이름이 없어짐(?)
	}
	
	
	//Statment 객체는 변수값을 처리하는 것이 복잡하다. PareparedStatement를 사용한다.
	...
	//out.println (sql);
	//if(true) return ; 		//프로그램을 중지시킴. 디버깅할 때 사용함
	
	stmt.executeUpdate(sql);	//DB 저장 완료, commit을 자동으로 처리
	
	}catch(Exception e) {
		out.println("예상치 못한 오류가 발생했습니다.<p/>");
		out.println("고객 센터 : 02-1111-1111 <p/>");
		//e.printStackTrace();   // 디버깅할 때 콘솔에서 오류확인 용도
	}finally {
		if( conn != null) conn.close();
		if (stmt != null) stmt.close();
		if (rs != null) rs.close();
	}
	// try catch 블락으로 프로그램이 종료되지 않도록 처리 후 객체 제거	
%>
