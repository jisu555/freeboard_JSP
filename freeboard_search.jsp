...
//Vector 변수 선언
Vector name = new Vector();		//name 커럼의 모든 값을 저장하는 Vector
...
//페이징을 처리할 변수 선언
	...
	int maxrow = 5; 			//한 페이지 내에 출력할 행의 갯수 (row, 행, 레코드 갯수)
...
%>
	<tr align="center" bgcolor="#87E8FF"> 
	  ...
	  <td width="340" bgcolor="#DFEDFF"><font size="2"><a href="freeboard_read.jsp?id=<%= keyid.elementAt(j)%>&page=<%= where%>"><%= subject.elementAt(j) %></a></font></td>
...
<%
	if (wheregroup > 1) { //현재 나의 그룹이 1 이상일 때 처음
		out.println("[<a href = 'freeboard_search.jsp?gogroup=1"+ "&stype=" + what + "&sval=" + val +"'> 처음 </a>]");
		out.println("[<a href = 'freeboard_search.jsp?gogroup=" + priorgroup + "&stype=" + what + "&sval=" + val +"'> 이전 </a>]");
	...
	if (name.size() != 0) { 	//name.size() : 총 레코드의 갯수가 0이 아니라면
		for (int jj=startpage; jj<=endpage; jj++){
			if (jj == where) {
				...
				out.println("[<a href='freeboard_search.jsp?go=" + jj + "&stype=" + what + "&sval=" + val +"'>"+ jj + "</a>]" );
			}
		}
	}
	// [다음][마지막]
	if (wheregroup < totalgroup) { //링크를 처리
		out.println("[<a href='freeboard_search.jsp?gogroup=" + nextgroup + "&stype=" + what + "&sval=" + val +"'>다음</a>]");
		out.println("[<a href='freeboard_search.jsp?gogroup=" + totalgroup + "&stype=" + what + "&sval=" + val +">마지막]</a>");
