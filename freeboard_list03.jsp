<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <!-- 페이징 처리 부분 추가된 freeboard_list03 -->

<!-- 클래스 import, DB Connection객체 -->
...
<!-- JSP 코드 블락 : DB의 레코드를 가져와서 루프 : 시작 -->
	...
	//go 변수를 넘겨받아서 wheregroup, startpage, endpage 정보를 알아낼 수 있다.
		//코드 블락 
	...
	int id = 0;		// DB의 id컬럼의 값을 가져오는 변수
	String em = null;  //DB에서 mail 주소를 가져와서 처리하는 변수    <== //
	
		//SQl 쿼리를 보낼 때 객체 변수 선언
		String sql = null;
		Statement stmt = null;
		ResultSet rs = null;
		
		//리스트페이지에서 답변글 처리의 출력을 하기 위한 쿼리
		...
		out.println ("<p/> 전체 페이지 갯수 (totalpages : ) : " + totalpages + "<p/>");
	...
	//해당 페이지를 처리하면서 해당 페이지에 대한 내용을 출력 (rs의 값을 vector에 저장했으므로 for문)
